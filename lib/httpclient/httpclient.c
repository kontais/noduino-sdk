...
static void ICACHE_FLASH_ATTR receive_callback(void *arg, char *buf,
					       unsigned short len)
{
...
static void ICACHE_FLASH_ATTR sent_callback(void *arg)
{
	...
	if (req->post_data == NULL) {
		...
		if (req->secure)
			espconn_secure_sent(conn, (uint8_t *) req->post_data,
					    strlen(req->post_data));
		else
			espconn_sent(conn, (uint8_t *) req->post_data,
				     strlen(req->post_data));
...
static void ICACHE_FLASH_ATTR connect_callback(void *arg)
{
	...
	char buf[69 + strlen(method) + strlen(req->path) +
	...
	int len = os_sprintf(buf,
			     ...
			     "Connection: close\r\n"
			     "User-Agent: ESP8266\r\n" "%s" "%s" "\r\n",
			     method, req->path, req->hostname, req->port,
				 ...
				 http_callback user_callback)
{
	...
	PRINTF("port=%d\n", port);
	PRINTF("path=%s\n", path);
	http_raw_request(hostname, port, secure, path, post_data, headers,
			 user_callback);
}
